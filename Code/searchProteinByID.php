...
$query = "SELECT CONCAT('https://www.nextprot.org/entry/', SUBSTRING_INDEX(SUBSTRING_INDEX(t.IRI, '#', 2), '#', -1)) AS IRI,
             COUNT(t.IRI) as NanoCount, t.IRI as OrignalIRI, np.dataset, t.rdfsLabel as fullName, t.rdfsDescription as description FROM topic t 
                INNER JOIN nanopubs np on t.npId = np.npId 
                WHERE t.rdfsLabel is NOT null and 
                      SUBSTRING_INDEX(SUBSTRING_INDEX(t.IRI, '#', 2), '#', -1) like '%$search%'
                GROUP BY t.IRI
