...
class SSP {
	...
	static function data_output_merge ( $columns, $data, $exactArray )
	{
		//echo 'from output';
		$out = array();
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
	...
	static function ims_merge ( $request, $conn, $table, $primaryKey, $columns, $exactArray )
	{
		$bindings = array();
		$db = self::db( $conn );
		...
		return array(
			...
			"recordsTotal"    => intval( $recordsTotal ),
			"recordsFiltered" => intval( $recordsFiltered ),
			"data"            => self::data_output_merge( $columns, $data, $exactArray )
		);
	}
