...
<html>
    <head>
        ...
        <title>Topic Mapping By All Datasets</title>
    </head>
    <body>
    ...
    $('#divTable').hide();
     //////////////////////////////////////////////////////////////////////////////////
        ...
        var end;
        if (localStorage.getItem('topicByAllDataset') != null) {
            $('#message').html('');
            DisplayData(localStorage.getItem('topicByAllDataset'));
        }
        else
        {
            var xmlhttp = new XMLHttpRequest();
              xmlhttp.open("GET", "Code/getJSONTopicInfoByIRI.php");
              xmlhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
              xmlhttp.send();
              xmlhttp.onreadystatechange = function() {
                if (this.readyState === 4 && this.status === 200) {
                  let json_data = this.responseText;
                  localStorage.setItem('topicByAllDataset', json_data);
                  /////////////////////////////
        ...
        let parseJson = JSON.parse(json);
                  ...
                  for(let i=0;i<parseJson.length;i++)
                  {
                           ...
                           let resolveIRI = IRI;
                            ...
                            table += `<tr>
                                        <td><a href="${resolveIRI}" target="_blank">${parseJson[i].IRI}</a></td>
                                        <td style="cursor:pointer;text-decoration:underline;color:#007bff" onclick="AJAXCallForNano('${IRI}', ${parseJson[i].GroupCount}, '', '${resolveIRI}')">${parseJson[i].GroupCount}</td>
                                    </tr>`;
                            array.push({'Name': parseJson[i].IRI, 'Count': parseJson[i].GroupCount, 'IRI': resolveIRI});
                  }
                  $('#tblTopic').html(table);
                  $('#divTable').show();
                  $('#tbl').DataTable();
    ...
    function millisToMinutesAndSeconds(millis) {
        ...
        var node = svg.selectAll(".node")
            ...
            .on("click", function(d){
                AJAXCallForNano(d.data.Name, d.data.Count, '', d.data.IRI);
                //alert(d.data.Name);
            })
