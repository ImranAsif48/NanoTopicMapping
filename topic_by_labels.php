...
<html>
    <head>
        ...
        <title>Topic Mapping By All Datasets Labels</title>
    </head>
    <body>
    ...
    $('#divTable').hide();
     //////////////////////////////////////////////////////////////////////////////////
        ...
        var end;
        if (localStorage.getItem('topicByLabels') != null) {
            $('#message').html('');
            DisplayData(localStorage.getItem('topicByLabels'));
        }
        else
        {
            var xmlhttp = new XMLHttpRequest();
              xmlhttp.open("GET", "Code/getJSONTopicLabel.php");
              xmlhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
              xmlhttp.send();
              xmlhttp.onreadystatechange = function() {
                if (this.readyState === 4 && this.status === 200) {
                  let json_data = this.responseText;
                  localStorage.setItem('topicByLabels', json_data);
                  /////////////////////////////
    ...
    function DisplayData(json)
    {
        let parseJson = JSON.parse(json);
                  ...
                  {
                            table += `<tr>
                                        <td>${parseJson[i].Label}</td>
                                        <td style="cursor:pointer;text-decoration:underline;color:#007bff" onclick="getNanopublitions('${parseJson[i].Label}', ${parseJson[i].GroupCount})">${parseJson[i].GroupCount}</td>
                                    </tr>`;
                            array.push({'Name': parseJson[i].Label, 'Count': parseJson[i].GroupCount});
                  }
                  $('#tblTopic').html(table);
                  $('#divTable').show();
                  $('#tbl').DataTable();
